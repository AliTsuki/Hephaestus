...
public static class Extensions
{
    ...
    public static Vector3Int ChunkPosToWorldPos(this Vector3Int chunkPos)
    {
        ...
        {
            x = chunkPos.x * GameManager.ChunkSize,
            y = chunkPos.y * GameManager.ChunkSize,
            z = chunkPos.z * GameManager.ChunkSize
        };
        return worldPos;
    }
    ...
    public static Vector3Int InternalPosToWorldPos(this Vector3Int internalPos, Vector3Int chunkPos)
    {
        Vector3Int worldPos = new Vector3Int
        {
            x = internalPos.x + (chunkPos.x * GameManager.ChunkSize),
            y = internalPos.y + (chunkPos.y * GameManager.ChunkSize),
            z = internalPos.z + (chunkPos.z * GameManager.ChunkSize)
        };
        return worldPos;
    }
    ...
    {
        Vector3Int chunkPos = new Vector3Int(worldPos.x / GameManager.ChunkSize, worldPos.y / GameManager.ChunkSize, worldPos.z / GameManager.ChunkSize);
        if(worldPos.x < 0 && worldPos.x % GameManager.ChunkSize < 0)
        {
            chunkPos.x -= 1;
        }
        if(worldPos.y < 0 && worldPos.y % GameManager.ChunkSize < 0)
        {
            chunkPos.y -= 1;
        }
        if(worldPos.z < 0 && worldPos.z % GameManager.ChunkSize < 0)
        {
            chunkPos.z -= 1;
        }
        return chunkPos;
    ...
    {
        Vector3Int internalPos = new Vector3Int(worldPos.x / GameManager.ChunkSize, worldPos.y / GameManager.ChunkSize, worldPos.z / GameManager.ChunkSize);
        if(worldPos.x < 0 && worldPos.x % GameManager.ChunkSize < 0)
        {
            internalPos.x -= 1;
        }
        internalPos.x *= GameManager.ChunkSize;
        if(worldPos.y < 0 && worldPos.y % GameManager.ChunkSize < 0)
        {
            internalPos.y -= 1;
        }
        internalPos.y *= GameManager.ChunkSize;
        if(worldPos.z < 0 && worldPos.z % GameManager.ChunkSize < 0)
        {
            internalPos.z -= 1;
        }
        internalPos.z *= GameManager.ChunkSize;
